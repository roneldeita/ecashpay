              ...
              <Form onSubmit={onSubmit} autoComplete="off">
                <FormItem
                  ...
                  {getFieldDecorator('Code', {
                    ...
                    <Input onKeyUp={onSubmit} className="verification"/>
                  )}
                </FormItem>
                <FormItem>
                  <Button className="submit" type="primary" htmlType="submit" loading={buttonState} block>{buttonState ? 'Verifying..' : 'Submit'}</Button>
                </FormItem>
              </Form>
              <p>Haven{`'`}t received a verification?
                {resendState ?
                  <span style={{color:'#999999'}}> Resending...</span> :
                  <span stlye={{color:'#1890ff'}} onClick={onResend}> Resend</span>}
              </p>
            </Col>
